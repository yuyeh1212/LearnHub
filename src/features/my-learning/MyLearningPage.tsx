    ...
    <div className="my-learning-page">
      ...
      <div className="my-learning-shell">
        ...
        <main id="my-learning-content">
          {isRestoringSession || (authUser && isLoading) ? <section className="my-learning-state" role="status"><span className="my-learning-state__pulse" aria-hidden="true" /><h1>正在整理你的學習紀錄</h1><p>課程、單元與進度會在這裡一起出現。</p></section> : null}

          {!isRestoringSession && !authUser ? <section className="my-learning-guest" aria-labelledby="learning-guest-title"><p className="eyebrow">你的學習空間</p><h1 id="learning-guest-title">把每次前進，留在同一條路上。</h1><p>登入後即可查看已加入的課程、保存觀看位置，並從上次停下的單元繼續。</p><div><button className="learning-primary-action" type="button" onClick={onSignIn}>登入查看學習紀錄</button><button className="learning-secondary-action" type="button" onClick={onRegister}>建立免費帳戶</button></div></section> : null}
          ...
          {!isRestoringSession && authUser && !isLoading && !error && featuredCourse ? <>
            <section className="my-learning-hero" aria-labelledby="my-learning-title">
              <div><p className="eyebrow">歡迎回來，{authUser.displayName}</p><h1 id="my-learning-title">從上次停下的地方，接著走。</h1><p>你不需要重新找路。最近學習、完成進度和下一個單元都已經整理好了。</p></div>
            ...
            <section className="learning-focus" aria-labelledby="learning-focus-title">
              <div className="learning-focus__route" aria-hidden="true"><span className="is-done">加入課程</span><i /><span className="is-active">持續學習</span><i /><span>完成目標</span></div>
            ...
            <section className="learning-library" aria-labelledby="learning-library-title"><div className="learning-library__heading"><div><p className="eyebrow">已加入的課程</p><h2 id="learning-library-title">你的學習清單</h2></div><a href="#explore">探索更多課程 →</a></div><div className="learning-course-grid">{learningCourses.map((learningCourse) => <LearningCourseCard key={learningCourse.enrollment.id} learningCourse={learningCourse} />)}</div></section>
          </> : null}
        </main>
      </div>
      <footer className="my-learning-footer"><strong>LearnHub</strong><p>每次回來，都知道下一步在哪裡。</p><a href="#home">回到課程首頁</a></footer>
    </div>
  )
}
