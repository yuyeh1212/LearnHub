...
LearnHub 使用 PostgreSQL。初始結構定義於 `db/migrations/0001_learning_core.sql`，學習狀態擴充定義於 `db/migrations/0002_course_learning_states.sql`，私有內容物件定義於 `db/migrations/0003_private_content_assets.sql`；migration 目前仍需依序手動執行。
...
`db/seeds/local-demo-content.sql` 只供本機開發展示課程目錄與播放器。它不含帳號、密碼或選課紀錄，也不應套用到正式環境。要讓首頁與播放器取得資料，請先依序完成三個 migration、執行種子檔，再執行 `scripts/prepare-local-content.ps1` 準備被 Git 忽略的本機私有檔案。
